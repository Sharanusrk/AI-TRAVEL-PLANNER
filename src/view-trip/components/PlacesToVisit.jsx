...
const PlacesToVisit = ({ trip }) => {
  return (
    <div className="p-4">
      <h2 className="font-bold text-lg mb-4 text-center sm:text-left">Places to Visit</h2>
        ...
        {trip?.tripData?.itinerary &&
          Object.keys(trip.tripData.itinerary).map((dayKey, index) => (
            <div key={index} className="bg-gray-100 p-4 rounded-xl shadow">
              ...
              <h3 className="font-semibold text-lg mb-3 text-center sm:text-left">Day {index + 1}</h3>

              {/* Loop through morning, afternoon, evening, dinner */}
              <div className="grid md:grid-cols-2 gap-6 text-center sm:text-left">
